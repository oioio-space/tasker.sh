...
| `exemples/collecte-linux.conf` | une collecte de triage complète sur une image montée — 27 étapes, 29 selon l'image : système, paquets avec leur date d'installation et l'historique du gestionnaire de paquets, comptes et droits, connexions (`wtmp`, `btmp`, `lastlog`, et `wtmp.db`/`lastlog2.db` quand l'image les a), réseau, `/var/log` et le journal systemd exporté en clair quand il est sur disque, persistance, timeline, six étapes rejouées pour chaque compte lu dans le `/etc/passwd` de l'image, disques virtuels, photorec. Les pièces sont copiées (`cp -a`, dates conservées) ou mises en lots (`tar`), pas seulement lues ; l'outil est choisi d'après l'image — `rpm` ou `dpkg-query`, `tune2fs` et la Sleuth Kit sur ext, `blkid` et `find` ailleurs, `journalctl` et `sqlite3` seulement s'il y a de quoi les nourrir — et il n'est exigé que là |
