...
| `exemples/collecte-linux.conf` | une collecte de triage complète sur une image montée — 26 étapes : distribution, fuseau, `fstab`, paquets `dpkg` ou `rpm`, `wtmp`/`btmp` copiés puis relus, profils réseau, `/var/log`, timeline, cinq étapes rejouées pour chaque compte, disques virtuels, photorec. Le portage d'un script de collecte écrit d'une traite, en commandes simples — ni fonction, ni sed, ni awk |
