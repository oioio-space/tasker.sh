...
| `,ecran` | l'étape occupe l'écran (photorec) ou parle à l'opérateur : pas de témoin d'attente |
...
| `exemples/collecte-linux.conf` | une collecte de triage complète sur une image montée — 24 étapes, jusqu'à 33, et davantage encore quand l'image porte plusieurs volumes, selon ce qu'elle contient et ce qui est installé : système, paquets avec leur date d'installation et l'historique du gestionnaire de paquets, comptes, droits et appartenance à un domaine (sssd, kerberos, samba, et le cache sss qui nomme les comptes du domaine vus sur la machine), identité de la machine et journaux de l'installateur qui la datent, connexions (`wtmp`, `btmp`, `lastlog`, et `wtmp.db`/`lastlog2.db` quand l'image les a), réseau, `/var/log` et le journal systemd exporté en clair quand il est sur disque, persistance, timeline, quatre à six étapes rejouées pour chaque compte lu dans le `/etc/passwd` de l'image, disques virtuels, photorec. Chaque volume monté sous l'image — un `/home` sur un autre volume — a droit au même traitement que la racine : son corps de timeline lu sur son périphérique, une seule chronologie en sortant, son système de fichiers, son `xfs_undelete`, son photorec. Les pièces sont copiées (`cp -a`, dates conservées) ou mises en lots (`tar`), pas seulement lues ; l'outil est choisi d'après l'image — `rpm` ou `dpkg-query`, `tune2fs` et la Sleuth Kit sur ext, `fsxfsinfo` sur xfs quand il accepte le volume, `blkid` et `find` ailleurs, `xfs_undelete` s'il est installé, `journalctl`, `sqlite3`, `last` et `lastb` seulement s'il y a de quoi les nourrir — et il n'est exigé que là. Une étape sans matière n'existe pas, plutôt que d'échouer. Pour l'exploiter ensuite, `exemples/skills/forensic-linux/` en tire un rapport daté et sourcé — voir `exemples/skills/README.md` |
...
| `TK_TEMOIN` | secondes avant le témoin d'attente : un segment qui glisse sur la dernière ligne du terminal, qu'il se réserve — la sortie défile au-dessus sans être touchée ; `,ecran` l'écarte d'une étape | entier ≥ 0, `0` = jamais |
...
| `-y`, `--yes` | | répondre le défaut à chaque question ; une valeur libre encore vide n'en a pas et reste demandée |
...
* `head` ferme le tube dès qu'il a ses lignes : ce qui est devant — `tee`, `ls` — meurt d'un SIGPIPE, code 141, et un `tee` peut mourir avant d'avoir tout écrit. Derrière un `tee` ou un producteur dont le code compte, lisez jusqu'au bout : `tail -n N` pour les dernières lignes, `sed -n '1,Np'` pour les premières.
