...
| `exemples/collecte-linux.conf` | une collecte de triage complète sur une image montée — 22 étapes, jusqu'à 31, et davantage encore quand l'image porte plusieurs volumes, selon ce qu'elle contient et ce qui est installé : système, paquets avec leur date d'installation et l'historique du gestionnaire de paquets, comptes et droits, connexions (`wtmp`, `btmp`, `lastlog`, et `wtmp.db`/`lastlog2.db` quand l'image les a), réseau, `/var/log` et le journal systemd exporté en clair quand il est sur disque, persistance, timeline, quatre à six étapes rejouées pour chaque compte lu dans le `/etc/passwd` de l'image, disques virtuels, photorec. Chaque volume monté sous l'image — un `/home` sur un autre volume — a droit au même traitement que la racine : son corps de timeline lu sur son périphérique, une seule chronologie en sortant, son système de fichiers, son `xfs_undelete`, son photorec. Les pièces sont copiées (`cp -a`, dates conservées) ou mises en lots (`tar`), pas seulement lues ; l'outil est choisi d'après l'image — `rpm` ou `dpkg-query`, `tune2fs` et la Sleuth Kit sur ext, `fsxfsinfo` sur xfs quand il accepte le volume, `blkid` et `find` ailleurs, `xfs_undelete` s'il est installé, `journalctl`, `sqlite3`, `last` et `lastb` seulement s'il y a de quoi les nourrir — et il n'est exigé que là. Une étape sans matière n'existe pas, plutôt que d'échouer |
