...
Un fichier par volume monté sous le point de montage de l'analyse, la racine
comme un `/home` posé sur un autre disque. La liste est celle qui sert à
...
**`strings` est appelé NU, sans une seule option, et la collecte ne cherche
RIEN dedans.** La pièce est le texte brut du périphérique tel que l'outil le
rend par défaut, et un lecteur qui veut la refaire tape exactement la même
commande. La recherche se fait ensuite, sur la pièce, par
`extraire.py --indicateurs` — voir « Comment y chercher » plus bas.

Une collecte plus ancienne peut porter à côté cinq extraits déjà triés
(`_urls`, `_courriels`, `_ip`, `_mac`, `_chemins`), au format `compte valeur`.
L'extracteur les lit toujours quand ils sont là ; la collecte ne les produit
plus.
