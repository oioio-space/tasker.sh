...
testée.
"""
import io, json, os, shutil, sqlite3, struct, sys, tarfile, tempfile, calendar, time
...
U = struct.Struct("<ii32s4s32s256shhiii16s20s")
def utmp(entries):
    out = b""
    for typ, pid, tty, user, host, iso in entries:
        out += U.pack(typ, pid, tty.encode().ljust(32, b"\0"), tty[-4:].encode().ljust(4, b"\0"),
...
def base(P, osrel, tz="Europe/Paris"):
    ...
    tarz(os.path.join(R, f"COMPTES/{P}_droits.tar.gz"), {"etc/shadow": "root:$6$a$b:19000:0:99999:7:::\nalice:$6$c$d:19800:0:90:7:::\n"})
    tarz(os.path.join(R, f"COMPTES/{P}_alice_artefacts.tar.gz"), {".bash_history": "ls\n#1767700000\nsudo -u bob true\n", ".config/chromium/Default/History": sqlite_blob(
    ...
    tarz(os.path.join(R, f"PERSISTANCE/{P}_persistance.tar.gz"), {"etc/crontab": "0 3 * * * root /usr/bin/updatedb\n"})
    return R, w
...
P = "PC01_A1_MAT_fedora"; R, w = base(P, 'ID=fedora\nVERSION_ID=40\nPRETTY_NAME="Fedora Linux 40 (Workstation Edition)"\n')
...
tarz(os.path.join(R, f"PAQUETS/{P}_historique.tar.gz"), {"usr/lib/sysimage/libdnf5/transaction_history.sqlite": dnf5, "var/log/dnf5.log": "2026-01-03T09:22:31+0000 INFO Installed: kodi\n"})
wdb = sqlite_blob(["CREATE TABLE wtmp(ID INTEGER PRIMARY KEY, Type INTEGER, User TEXT, Login INTEGER, Logout INTEGER, TTY TEXT, RemoteHost TEXT, Service TEXT)"],
                  [("INSERT INTO wtmp VALUES(?,?,?,?,?,?,?,?)", [(1, 2, "alice", epoch("2026-01-06T08:05:00") * 1000000, epoch("2026-01-06T17:10:00") * 1000000, "tty2", "", "gdm"),
                                                                   (2, 2, "alice", epoch("2026-01-10T14:00:00") * 1000000, None, "pts/0", "10.0.0.9", "sshd")])])
...
tarz(os.path.join(R, f"JOURNAUX/{P}_var_log.tar.gz"), {"var/log/dnf.log": "x\n"})
tarz(os.path.join(R, f"RESEAU/{P}_reseau.tar.gz"), {"etc/NetworkManager/system-connections/Maison.nmconnection": "[connection]\nid=Maison\nuuid=aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa\ntype=wifi\n\n[wifi]\nssid=Livebox-Maison\n",
...
P = "PC02_A1_MAT_debian"; R, w = base(P, 'ID=debian\nVERSION_ID="13"\nPRETTY_NAME="Debian GNU/Linux 13 (trixie)"\n')
...
tarz(os.path.join(R, f"PAQUETS/{P}_historique.tar.gz"), {"var/log/dpkg.log": "2025-06-01 08:00:00 install base-files:amd64 <none> 13.1\n2026-01-04 18:30:00 install transmission-gtk:amd64 <none> 4.0.6-1\n",
                                                          ...
                                                          "var/log/apt/history.log.1.gz": __import__("gzip").compress(b"Start-Date: 2025-12-01  10:00:00\nCommandline: apt remove kodi\nRemove: kodi:amd64 (20.5)\nEnd-Date: 2025-12-01  10:00:04\n")})
w("CONNEXIONS/wtmp.db", sqlite_blob(["CREATE TABLE wtmp(ID INTEGER PRIMARY KEY, Type INTEGER, User TEXT, Login INTEGER, Logout INTEGER, TTY TEXT, RemoteHost TEXT, Service TEXT)"],
                  [("INSERT INTO wtmp VALUES(?,?,?,?,?,?,?,?)", [(1, 2, "alice", epoch("2026-01-04T17:50:00") * 1000000, epoch("2026-01-04T19:00:00") * 1000000, "tty2", "", "login")])]))
tarz(os.path.join(R, f"JOURNAUX/{P}_var_log.tar.gz"), {"var/log/auth.log": "Jan  4 18:29:50 pc02 sudo:    alice : TTY=pts/0 ; PWD=/home/alice ; USER=root ; COMMAND=/usr/bin/apt install transmission-gtk\nJan  4 18:40:00 pc02 su: (to bob) alice on pts/1\nJan  4 18:40:00 pc02 su: pam_unix(su:session): session opened for user bob(uid=1001) by alice(uid=1000)\n",
                                                        "var/log/syslog.1.gz": __import__("gzip").compress(b"Dec 30 08:00:00 pc02 kernel: usb 2-1: New USB device found, idVendor=0951, idProduct=1666\nDec 30 08:00:00 pc02 kernel: usb 2-1: SerialNumber: 0019E06B0001\n")})
tarz(os.path.join(R, f"RESEAU/{P}_reseau.tar.gz"), {"etc/wpa_supplicant/wpa_supplicant.conf": 'ctrl_interface=/run/wpa_supplicant\nnetwork={\n    ssid="McDonalds Free WiFi"\n    key_mgmt=NONE\n}\nnetwork={\n    ssid="Bureau-5G"\n    psk="secret"\n}\n',
...
tarz(os.path.join(R, f"RESEAU/{P}_reseau.tar.gz"), {"etc/NetworkManager/system-connections/Gare.nmconnection": "[connection]\nid=Gare\nuuid=bbbbbbbb-bbbb-bbbb-bbbb-bbbbbbbbbbbb\ntype=wifi\npermissions=user:alice:;\n\n[wifi]\nssid=SNCF_WIFI\n", "etc/ssh/sshd_config": "PermitRootLogin yes\n", "etc/firewalld/zones/public.xml": "<zone/>\n"})
...
P = "PC05_A1_MAT_alpine"; R, w = base(P, 'ID=alpine\nVERSION_ID=3.21.0\nPRETTY_NAME="Alpine Linux v3.21"\n')
w(f"PAQUETS/{P}_paquets.txt", "busybox-1.37.0-r9\ntransmission-cli-4.0.6-r0\n")
w(f"PAQUETS/{P}_apk_installed.txt", "C:Q1abc\nP:busybox\nV:1.37.0-r9\nA:x86_64\n\nC:Q1def\nP:transmission-cli\nV:4.0.6-r0\nA:x86_64\n\n")
tarz(os.path.join(R, f"PAQUETS/{P}_historique.tar.gz"), {"etc/apk/world": "alpine-base\nbusybox\ntransmission-cli\n", "etc/apk/repositories": "https://dl-cdn.alpinelinux.org/alpine/v3.21/main\n"})
tarz(os.path.join(R, f"JOURNAUX/{P}_var_log.tar.gz"), {"var/log/messages": "Jan  6 21:00:00 pc05 daemon.info sudo: alice : TTY=pts/0 ; PWD=/home/alice ; USER=root ; COMMAND=/sbin/apk add transmission-cli\nJan  6 21:01:00 pc05 auth.info sshd[12]: Accepted publickey for alice from 10.0.0.7 port 5000 ssh2\n"})
tarz(os.path.join(R, f"RESEAU/{P}_reseau.tar.gz"), {"etc/wpa_supplicant/wpa_supplicant.conf": 'network={\n    ssid="ibis-hotel-wifi"\n}\n', "etc/ssh/sshd_config": "PermitRootLogin prohibit-password\n"})
